...
def benchmark_conversion_latency():
    ...
    print("=" * 65)
    print(
        f"{'Matrix Elements':<18} | {'v0.2.0 (Copy)':<16} | {'v0.3.0 (TensorView)':<20} | {'Speedup':<10}"
    )
    print("-" * 65)
    ...
    for rows, cols in sizes:
        ...
        print(
            f"{n_elems:<18,d} | {t_v02:>13.2f} ms | {t_v03:>17.4f} ms | {speedup:>8.1f}x"
        )
...
def benchmark_ingestion_throughput():
    ...
    print(
        f"v0.2.0 Python Slicing : {v02_rate:12,.0f} samples/sec ({t_v02*1000:.1f} ms)"
    )
    print(
        f"v0.3.0 HPC DataLoader : {v03_rate:12,.0f} samples/sec ({t_v03*1000:.1f} ms)"
    )
...
def generate_comparison_plots(
    ...
    ax1.plot(
        ...
        linewidth=2.5,
        markersize=8,
        label="v0.2.0: Deep Copy (vector<vector>)",
    )
    ax1.plot(
        elements,
        v03_latency,
        ...
        linewidth=2.5,
        markersize=8,
        label="v0.3.0: TensorView (Zero-Copy)",
    )
    ax1.set_yscale("log")
    ax1.set_xlabel(
        "Number of Matrix Elements", color=text_color, fontsize=11, fontweight="bold"
    )
    ax1.set_ylabel(
    ...
    ax1.set_title(
        "1. Buffer Ingestion Latency (Lower is Better)",
        color=text_color,
        fontsize=13,
        fontweight="bold",
        pad=12,
    ...
    ax2 = axes[0, 1]
    elem_labels = [
        f"{e // 1000}k" if e < 1_000_000 else f"{e / 1_000_000:.1f}M" for e in elements
    ]
    ...
    ax2.bar(
        ...
        v02_memory,
        width,
        label="v0.2.0: Heap Duplication",
        color=color_v02,
        alpha=0.9,
    )
    ax2.bar(
        x_indices + width / 2,
        [0.1 for _ in v03_memory],
        width,
        label="v0.3.0: Zero Heap Overhead",
        color=color_v03,
        alpha=0.9,
    )
    ...
    ax2.set_xlabel(
        "Matrix Size (Elements)", color=text_color, fontsize=11, fontweight="bold"
    )
    ax2.set_ylabel(
        "Additional RAM Allocated (MB)",
        color=text_color,
        fontsize=11,
        fontweight="bold",
    )
    ax2.set_title(
        "2. Memory Footprint Overhead (Lower is Better)",
        color=text_color,
        fontsize=13,
        fontweight="bold",
        pad=12,
    ...
    bars = ax3.bar(
        methods, rates, color=[color_v02, color_v03], width=0.55, edgecolor=grid_color
    )
    ax3.set_ylabel(
        "Throughput (Million Samples / Sec)",
        color=text_color,
        fontsize=11,
        fontweight="bold",
    )
    ax3.set_title(
        "3. Data Ingestion Throughput (Higher is Better)",
        color=text_color,
        fontsize=13,
        fontweight="bold",
        pad=12,
    ...
    ax4.set_title(
        "4. Feature & Architectural Capabilities",
        color=text_color,
        fontsize=13,
        fontweight="bold",
        pad=12,
    ...
    plt.suptitle(
        "MorphosML: Evolution from v0.2.0 to v0.3.0 (HPC Architecture)",
        color="#38bdf8",
        fontsize=16,
        fontweight="bold",
        y=0.98,
